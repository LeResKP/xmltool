...
class TestDTD(TestCase):
    def test_content(self):
        url = "https://raw.github.com/LeResKP/" "xmltool/master/tests/exercise.dtd"
        dtd_obj = dtd.DTD(url)
    ...
    def test_content_cache(self):
        ...
        with mock.patch("xmltool.cache.CACHE_TIMEOUT", 3600):
            with mock.patch(
                "xmltool.dtd.DTD._fetch", new=lambda self: fake_fetch(self, content)
            ):
                self.assertEqual(dtd_obj.content, content)

            with mock.patch(
                "xmltool.dtd.DTD._fetch",
