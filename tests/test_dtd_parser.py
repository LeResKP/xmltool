...
class DtdParser(TestCase):
    ...
    def test_dtd_to_dict(self):
        expected = {
            ...
            "Movie": {
                "elts": ("is-publish?,name,year,directors," "actors,resume?,critique*"),
                "attrs": [],
            },
    ...
    def test_dtd_to_dict_attrs(self):
        expected = {
            "comment": {"elts": "#PCDATA", "attrs": [("idcomment", "ID", "#IMPLIED")]},
            "question": {
                "elts": "#PCDATA",
                "attrs": [("idquestion", "ID", "#IMPLIED")],
            },
            "mqm": {"elts": "choice+", "attrs": [("idmqm", "ID", "#IMPLIED")]},
            "number": {"elts": "#PCDATA", "attrs": []},
            "comments": {
                "elts": "comment+",
                "attrs": [("idcomments", "ID", "#IMPLIED")],
            },
            "choice": {"elts": "#PCDATA", "attrs": [("idchoice", "ID", "#IMPLIED")]},
            "Exercise": {
                "elts": "number,test*",
                "attrs": [("idexercise", "ID", "#IMPLIED")],
            },
            "test": {
                "elts": "question,(qcm|mqm)*,comments?",
                "attrs": [("idtest", "ID", "#IMPLIED"), ("name", "PCDATA", "#IMPLIED")],
            },
            "qcm": {"elts": "choice+", "attrs": [("idqcm", "ID", "#IMPLIED")]},
        }
    ...
    def test__parse_elts(self):
        ...
        expected = [
            (
                ...
                True,
                True,
                [("tag1", True, False, []), ("tag2", True, False, [])],
            )
        ]
        self.assertEqual(lis, expected)
    ...
    def test__create_class_dict(self):
        ...
        self.assertEqual(dtd_dict, {"tag": {"elts": "tag1?,tag2?", "attrs": []}})
    ...
    def test__create_new_class(self):
        ...
        try:
            cls = dtd_parser._create_new_class(
                class_dic, "tag1_tag2", required=True, islist=False, conditionals=[]
            )
            assert False
        except Exception as e:
            self.assertEqual(
                str(e), "You should provide a base_cls or conditionals for tag1_tag2"
            )
